...
class EditNextResultView extends obsidian.ItemView {
    ...
    async onOpen() {
      ...
      .editnext-readability-high {
        background-color: rgba(100, 255, 100, 0.2);
        color: #388e3c;
      }
      .editnext-readability-medium {
        background-color: rgba(255, 200, 0, 0.2);
        color: #f57c00;
      }
      .editnext-readability-low {
        background-color: rgba(255, 100, 100, 0.2);
        color: #d32f2f;
      }
    `;
    ...
    populateTableRows(tbody) {
        ...
        for (const result of this.results) {
            ...
            // Add readability score with appropriate class and tooltip
            const readabilityCell = row.createEl('td');
            const tooltipContainer = readabilityCell.createEl('div', {
                cls: 'editnext-tooltip'
            });
            
            // Parse the readability score, defaulting to 0 if not present
            const readabilityScore = parseFloat(result.readability_score) || 0;
            const readabilityValue = readabilityScore.toFixed(1);
            const readabilityClass = this.getReadabilityClass(readabilityScore);
            
            tooltipContainer.createEl('span', {
                cls: `editnext-badge ${readabilityClass}`,
                text: readabilityValue
            });
            
            // Add tooltip text with explanation using the helper method
            const readabilityExplanation = this.getReadabilityExplanation(readabilityScore);
            
            tooltipContainer.createEl('span', {
                cls: 'editnext-tooltip-text',
                text: readabilityExplanation
            });
    ...
    getReadabilityClass(score) {
        // Handle undefined or null score values
    ...
    getReadabilityExplanation(score) {
        // Handle undefined or null score values
        score = Number(score) || 0;
        
        if (score >= 70) {
            return "Text is difficult to read (equivalent to college/professional level writing). Consider simplifying language and sentence structure.";
        } else if (score >= 40) {
            return "Text is moderately difficult (high school level). Some readers might find it challenging.";
        } else {
            return "Text is appropriately readable (middle school level or below). Good for general audience.";
        }
    }
